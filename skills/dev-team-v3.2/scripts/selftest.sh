...
check "every installed agent runs in dontAsk mode (a background lane never waits on a prompt)" '[ "$(grep -l "permissionMode: dontAsk" .claude/agents/*.md | wc -l)" -eq 5 ]'
check "every installed agent asks for the 1h prompt cache" '[ "$(grep -l "cacheTtl: 1h" .claude/agents/*.md | wc -l)" -eq 5 ]'
...
check "start prints the fast-mode tip" '[[ "$OUT" == *"/fast"* ]]'
cd "$RM"
