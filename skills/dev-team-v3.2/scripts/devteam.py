#!/usr/bin/env python3
"""dev-team engine v3.2 — deterministic scheduler, integrator and worktree helper.
    ...
    for sid, s, mode in blocks:
        ...
        if mode == "research":
            ...
            out(f"=== DISPATCH {sid} [RESEARCH] — {s['title'][:50]}",
                f"Agent → subagent_type: investigator, description: \"{sid}\", prompt: \"Read {brief} and follow it exactly.\"",
                "")
            continue
        model = dispatch_model(s)
        out(f"=== DISPATCH {sid} [{kind.upper()}/{mode.upper()}] — {s['title'][:50]}",
            f"Agent → subagent_type: programmer, description: \"{sid}\"" + (f", model: {model}" if model else "")
...
def cmd_doctor(a):
    ...
    for name in ("programmer", "code-reviewer", "spot-reviewer", "team-leader", "investigator"):
